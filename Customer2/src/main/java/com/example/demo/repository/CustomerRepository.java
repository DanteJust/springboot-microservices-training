...
public class CustomerRepository {
	...
	public Optional<Customer> findById(Long id) {
		return jdbcTemplate.query("SELECT * FROM customers where id = ?", rowMapper, id).stream().findFirst();
	}
	
	public int saveCustomer(Customer customer) {
		return jdbcTemplate.update("INSERT INTO customers(id, name, email) VALUES (?, ?, ?)",
	...
	public int updateCustomer(Long id, Customer customer) {
		return jdbcTemplate.update("UPDATE customers SET name=?, email=? WHERE id=?",
				customer.getName(), customer.getEmail(), id);
	}
